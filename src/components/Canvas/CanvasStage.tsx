...
export function CanvasStage() {
  ...
  const isPointInLayer = useCallback(
    (x: number, y: number, layer: Layer, asset: Asset): boolean => {
  ...
  const getResizeHandle = useCallback(
    (x: number, y: number, layer: Layer, asset: Asset): string | null => {
      if (!layer.visible || layer.transform.opacity <= 0) {
        return null;
      }
  ...
  const handlePointerMove = useCallback(
    (e: React.MouseEvent | React.TouchEvent) => {
      ...
      if (dragStateRef.current.isResizing && dragStateRef.current.resizeHandle) {
        ...
        const currentLayer = currentProject?.layers.find(
          (l) => l.id === dragStateRef.current.draggedLayerId
        );
      ...
      <div className={`canvas-container ${isDragOver ? 'drag-over' : ''}`}>
        <canvas
          ...
          style={{
            ...
            cursor: dragStateRef.current.isDragging
              ? 'grabbing'
              : dragStateRef.current.isResizing
                ? dragStateRef.current.resizeHandle?.includes('top-left') ||
                  dragStateRef.current.resizeHandle?.includes('bottom-right')
                  ? 'nw-resize'
                  : dragStateRef.current.resizeHandle?.includes('top-right') ||
                      dragStateRef.current.resizeHandle?.includes('bottom-left')
                    ? 'ne-resize'
                    : dragStateRef.current.resizeHandle?.includes('top-center') ||
                        dragStateRef.current.resizeHandle?.includes('bottom-center')
                      ? 'n-resize'
                      : dragStateRef.current.resizeHandle?.includes('center-left') ||
                          dragStateRef.current.resizeHandle?.includes('center-right')
                        ? 'e-resize'
                        : 'grab'
                : 'grab',
          }}
